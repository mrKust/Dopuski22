...
public class MarkovChain {
    ...
    public int[] processInTimeAndCountHeads(int startPosition, int T) {

        int finalHead = startPosition;
        int[] headCounts = new int[this.chain.size()];

        for (int i = 0; i < T; i++) {

            headCounts[finalHead]++;
            finalHead = this.processInTime(finalHead, 1);
        }

        return headCounts;
    }

    public int timeToN(int startPosition) {

        int steptsToPogloshenie = 0;
        int currentHead = startPosition;

        while (!checkHeadToPogloshenie(currentHead)) {

            steptsToPogloshenie++;
            currentHead = this.processInTime(currentHead, 1);
        }
        return steptsToPogloshenie;
    }

    private boolean checkHeadToPogloshenie(int numberOfHead) {

       List<Double> tmp = this.chain.get(numberOfHead).getProbabilitiesToSwitch();
